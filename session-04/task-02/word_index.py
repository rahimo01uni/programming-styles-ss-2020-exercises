...
class WordIndexManager:
    ...
    def __init__(self, event_manager):
        ...
        # Q: Can we replace those with a configuration provider that triggers 'configure' events? How can we ensure
        #   the correct "global" order when we run the main method?
        self._frequency_threshold = 100
        self._page_size = 45

        self._event_manager = event_manager
        self._event_manager.subscribe('eof', self.__produce_tagged_filtered_sorted_words)
        # Q: If this component subscribes twice, will it receive the event twice?
    ...
    def _filter(self):
        # Q: replace the following code with a one-liner that remove elements from the
...
class WordIndexFramework:
    ...
    def run(self, event):
        ...
        #   the registered components heve finished to handle the load message
        self._event_manager.publish(('load', path_to_file))

        # Otherwise, start would have to be sync (most likely on eof or similar?)
